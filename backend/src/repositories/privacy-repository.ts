...
export class PrivacyRepository {
  async recordConsent(userId: string, input: { purpose: string; granted: boolean }) {
    const { rows } = await db.query(
      'INSERT INTO consents (id, user_id, purpose, granted) VALUES ($1, $2, $3, $4) RETURNING id, purpose, granted, recorded_at AS "recordedAt"',
      [randomUUID(), userId, input.purpose, input.granted],
    );
    return rows[0];
  }
