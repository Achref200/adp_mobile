...
async function seedDemoData(client: PoolClient): Promise<boolean> {
  ...
  const insertUser = (id: string, email: string, first: string, last: string, country: string) =>
    client.query(
      'INSERT INTO users (id, email, first_name, last_name, country, password_hash, directory_visible) VALUES ($1,$2,$3,$4,$5,$6,TRUE) ON CONFLICT DO NOTHING',
      [id, email, first, last, country, hash],
    );
  ...
  const insertProfile = (userId: string, city: string, sector: string, skills: string[]) =>
    client.query(
      'INSERT INTO networking_profiles (user_id, city, sector, skills, visibility) VALUES ($1,$2,$3,$4,$5::jsonb) ON CONFLICT DO NOTHING',
      [userId, city, sector, skills, JSON.stringify({ visible: true })],
    );
  ...
  await client.query(
    `INSERT INTO memberships (id, user_id, plan, amount_cents, djerba_connection, motivation, status, submitted_at, paid_at, expires_at)
     VALUES ($1,$2,'benefactor',10000,$3,$4,'active',NOW(),NOW(),'2027-12-31') ON CONFLICT DO NOTHING`,
    [M_KARIM, U_KARIM, 'Originaire de Houmt Souk', 'Contribuer activement au rayonnement de Djerba'],
  );

  const insertProject = (id: string, title: string, category: string, summary: string, body: string, location: string, progress: number, target: number, raised: number, featured: boolean) =>
    client.query(
      ...
      [id, title, category, summary, body, location, progress, target, raised, featured],
    );
  await insertProject(
    P_GUELLALA,
    'Sauvegarde des Potiers Traditionnels de Guellala',
    'Patrimoine',
    "Modernisation respectueuse des ateliers ancestraux et mise en place d'une filière de distribution artisanale internationale.",
    "Le village de Guellala abrite des millénaires d'histoire potière. Ce projet finance des fours écologiques à haute efficacité thermique et un espace muséal coopératif pour préserver et pérenniser ce savoir-faire.",
    'Guellala, Djerba', 0.68, 2500000, 1700000, true,
  );

  await insertProject(
    P_ECOLOGIE,
    'Restauration de la Réserve Naturelle de Ras Rmel',
    'Écologie',
    'Protection des lagunes des flamants roses, nettoyage citoyen des côtes et balisage éco-responsable des sentiers maritimes.',
    "La presqu'île des flamants roses est l'un des écosystèmes les plus précieux de la Méditerranée. Ce programme déploie des barrières naturelles de protection des dunes et sensibilise les visiteurs.",
    'Ras Rmel, Djerba', 0.82, 1500000, 1230000, true,
  );
  await insertProject(
    P_EDUCATION,
    'Bibliothèque Digitale & FabLab Jeunesse',
    'Éducation',
    'Équipement numérique de trois écoles rurales et initiation au code et à la robotique pour 400 élèves djerbiens.',
    'Fourniture de tablettes éducatives, accès internet satellitaire sécurisé et ateliers animés par des ingénieurs de la diaspora bénévole.',
    'Ajim & Midoun, Djerba', 0.45, 2000000, 900000, false,
  );

  const insertNews = (id: string, title: string, excerpt: string, body: string, category: string) =>
    client.query(
      'INSERT INTO news (id, title, excerpt, body, category, published) VALUES ($1,$2,$3,$4,$5,TRUE) ON CONFLICT DO NOTHING',
      [id, title, excerpt, body, category],
    );
  await insertNews(
    N_NEWS_01,
    "L'UNESCO salue les initiatives citoyennes de préservation insulaire",
    "Le comité de suivi du patrimoine mondial souligne l'impact des chantiers participatifs menés par les associations et la diaspora à Djerba.",
    "Le classement de l'île de Djerba au patrimoine mondial de l'UNESCO en 2023 franchit une nouvelle étape grâce à la mobilisation des citoyens.",
    'Patrimoine',
  );
  await insertNews(
    N_NEWS_02,
    'Préparatifs officiels du Sommet International de la Diaspora 2026',
    'Plus de 350 délégués, investisseurs et personnalités culturelles sont attendus à Houmt Souk du 24 au 26 octobre 2026.',
    "Le programme plénier abordera le guichet unique d'investissement, l'écologie insulaire et les coopérations technologiques.",
    'Congrès',
  );

  const insertEvent = (id: string, title: string, summary: string, location: string, startsAt: string, endsAt: string) =>
    client.query(
      'INSERT INTO events (id, title, summary, location, starts_at, ends_at, published) VALUES ($1,$2,$3,$4,$5::timestamptz,$6::timestamptz,TRUE) ON CONFLICT DO NOTHING',
      [id, title, summary, location, startsAt, endsAt],
    );
  await insertEvent(
    E_SUMMIT,
    'Sommet International de la Diaspora 2026',
    "Trois journées de conférences, tables rondes d'investisseurs et rencontres institutionnelles.",
    'Houmt Souk, Djerba', '2026-10-24 09:00:00+01', '2026-10-26 18:00:00+01',
  );
  await insertEvent(
    E_AG,
    'Assemblée Générale Statutaire ADP 2026',
    'Vote des bilans financiers, renouvellement du bureau exécutif et validation des budgets de projets.',
    'Midoun, Djerba & Diffusion Live', '2026-11-14 14:00:00+01', '2026-11-14 18:00:00+01',
  );

  const insertNotification = (id: string, userId: string, type: string, title: string, body: string) =>
    client.query(
      'INSERT INTO notifications (id, user_id, type, title, body) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING',
      [id, userId, type, title, body],
    );
  await insertNotification(
    NOTIF_01, U_KARIM, 'summit',
    'Votre e-Pass pour le Sommet 2026 est disponible',
    "Accédez à votre QR code certifié dans l'onglet e-Pass pour l'accès prioritaire aux conférences.",
  );
  await insertNotification(
    NOTIF_02, U_KARIM, 'project',
    'Projet Potiers de Guellala : palier des 65% atteint !',
    'Grâce à votre soutien, les commandes des nouveaux fours céramiques écologiques ont été passées.',
  );

  return true;
}

export async function ensurePostgresSchema(pool: Pool): Promise<{ seeded: boolean }> {
