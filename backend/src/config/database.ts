...
export interface DbClient {
  ...
  connect(): Promise<{ query<T = any>(sql: string, params?: any[]): Promise<QueryResult<T>>; release(): void }>;
}
...
function createSqliteDb(): DbClient {
  ...
  try { sqlite.exec("ALTER TABLE users ADD COLUMN google_sub TEXT"); } catch { /* column already exists */ }
        ...
        if (isSelect) {
          ...
          for (const row of rows as any[]) {
            if (row && typeof row === 'object') {
              if (row.directoryVisible !== undefined) row.directoryVisible = Boolean(row.directoryVisible);
              if (row.anonymous !== undefined) row.anonymous = Boolean(row.anonymous);
              if (row.visible !== undefined) row.visible = Boolean(row.visible);
              if (row.skills && typeof row.skills === 'string') {
                try { row.skills = JSON.parse(row.skills); } catch { /* keep */ }
              }
              if (row.visibility && typeof row.visibility === 'string') {
                try { row.visibility = JSON.parse(row.visibility); } catch { /* keep */ }
              }
            }
          }
...
export async function databaseHealth(): Promise<'postgresql' | 'sqlite'> {
  await db.query('SELECT 1');
  return activeDb === (pgPool as any) ? 'postgresql' : 'sqlite';
}
