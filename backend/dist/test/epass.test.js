...
test('e-pass contract: issue, verify, and reject tampered payloads', async () => {
    ...
    await db.query('INSERT INTO users (id, email, first_name, last_name, country, password_hash) VALUES ($1,$2,$3,$4,$5,$6)', [userId, `${userId}@example.test`, 'Test', 'EPass', 'France', 'not-a-real-password-hash']);
    await db.query("INSERT INTO memberships (id, user_id, plan, amount_cents, djerba_connection, status, submitted_at, paid_at, expires_at) VALUES ($1,$2,$3,$4,$5,'active',NOW(),NOW(),$6)", [membershipId, userId, 'individual', 5000, 'Test connection', validUntil]);
    const app = buildApp();
