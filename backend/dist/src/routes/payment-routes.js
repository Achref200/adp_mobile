...
export async function paymentRoutes(app) {
    ...
    app.post('/checkout/donations', async (request, reply) => {
        ...
        await db.query("INSERT INTO donations (id, user_id, project_id, donor_email, anonymous, amount_cents, frequency, status) VALUES ($1,$2,$3,$4,$5,$6,$7,'pending')", [donationId, userId, input.projectId ?? null, input.donorEmail ?? null, input.anonymous, input.amountCents, input.frequency]);
        if (input.frequency === 'monthly') {
            if (!env.HELLOASSO_DONATION_FORM_URL)
                throw new AppError(503, 'recurring_payments_unavailable', 'The HelloAsso recurring donation form is not configured.');
    ...
    app.post('/checkout/memberships', { preHandler: requireAuth }, async (request, reply) => {
        ...
        if (!membership || membership.id !== membershipId)
            throw new AppError(404, 'membership_not_found', 'Membership request not found.');
    ...
    app.get('/checkout/status/:checkoutIntentId', { preHandler: requireAuth }, async (request) => {
        ...
        const ownership = await db.query(`SELECT user_id AS "userId" FROM donations WHERE helloasso_checkout_intent_id = $1
       UNION ALL SELECT user_id AS "userId" FROM memberships WHERE helloasso_checkout_intent_id = $1 LIMIT 1`, [checkoutIntentId]);
        if (ownership.rows[0]?.userId !== request.userId)
        ...
        const state = await db.query(`SELECT 'donation'::text AS kind, status::text FROM donations WHERE helloasso_checkout_intent_id = $1
       UNION ALL SELECT 'membership'::text AS kind, status::text FROM memberships WHERE helloasso_checkout_intent_id = $1 LIMIT 1`, [checkoutIntentId]);
    ...
    app.post('/helloasso/webhook', { config: { rawBody: true } }, async (request, reply) => {
        ...
        if (!env.HELLOASSO_WEBHOOK_SECRET || typeof signature !== 'string' || typeof raw !== 'string')
            throw new AppError(401, 'invalid_webhook', 'Webhook signature is required.');
        ...
        if (expected.length !== signature.length || !timingSafeEqual(Buffer.from(expected), Buffer.from(signature)))
            throw new AppError(401, 'invalid_webhook', 'Webhook signature is invalid.');
        const body = request.body;
        const eventId = createHash('sha256').update(raw).digest('hex');
        await db.query('INSERT INTO helloasso_webhook_events (event_id, event_type, payload) VALUES ($1,$2,$3) ON CONFLICT (event_id) DO NOTHING', [eventId, body.eventType ?? 'unknown', request.body]);
        await reconciliation.processWebhook(eventId);
        return reply.code(202).send({ accepted: true });
    });
}
function optionalUserId(request, tokens) {
